...
func (ns *nodeServer) dealPodVolumeMount(p *persistentVolumeWithPods, globalMountPath string) error {
	...
	// bind globalmount to pods， mount --bind -o remount 可以解决mount --bind多次挂载问题，但不能解决挂载点损坏问题，umount 会导致Pod在mountPropagation: HostToContainer模式下发生Transport endpoint is not connected，只能多次挂载
